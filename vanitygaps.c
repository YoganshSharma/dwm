...
bstack(Monitor *m)
{
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			resize(c, mx, my, mw * (c->cfact / mfacts) + (i < mrest ? 1 : 0) - (2*c->bw), mh - (2*c->bw), bw, 0);
			mx += WIDTH(c) + iv;
		} else {
			resize(c, sx, sy, sw * (c->cfact / sfacts) + ((i - m->nmaster) < srest ? 1 : 0) - (2*c->bw), sh - (2*c->bw), bw, 0);
			sx += WIDTH(c) + iv;
		}
	}
}
...
bstackhoriz(Monitor *m)
{
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, sx, sy, sw - (2*c->bw), sh * (c->cfact / sfacts) + ((i - m->nmaster) < srest ? 1 : 0) - (2*c->bw), bw, 0);
			sy += HEIGHT(c) + ih;
		}
	}
}
...
horizgrid(Monitor *m) {
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < ntop) {
			...
			resize(c, sx, sy, sw * (c->cfact / sfacts) + ((i - ntop) < srest ? 1 : 0) - (2*c->bw), sh - (2*c->bw), bw, 0);
			sx += WIDTH(c) + iv;
		}
}
...
nrowgrid(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next), ci++) {
		...
		resize(c, cx, cy, cw - (2*c->bw), ch - (2*c->bw), bw, 0);
	}
}
